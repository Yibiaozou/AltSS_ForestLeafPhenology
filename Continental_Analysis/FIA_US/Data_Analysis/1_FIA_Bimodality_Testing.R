...
saveRDS(b1zap, file="Data/FIA_GAM_ZAP_EV_FSD.rds")
saveRDS(b2zap, file="Data/FIA_GAM_ZAP_DE_FSD.rds")
...
saveRDS(AzapMx, file="Data/FIA_zapMx_EV_FSD.rds")
saveRDS(BzapMx, file="Data/FIA_zapMx_DE_FSD.rds")
...
toc()
saveRDS(b1zanusi, file="Data/FIA_GAM_ZANBI_EV_FSD.rds")
...
b2zanusi <- gamlss(de.density~ba(~s(PC1,k=kv)+s(PC2,k=kv)+s(PC3,k=kv)+s(PC4,k=kv)+s(PC5,k=kv)+s(PC6,k=kv)+s(PC7,k=kv)+s(PC8,k=kv)+s(PC9,k=kv)+s(PC10,k=kv)+s(Cluster,bs="re"), discrete=T,method="fREML"),
                   ...
                   family=ZANBI(),data=AX2, control=gamlss.control(c.crit = 0.05,n.cyc = 20))
toc()
saveRDS(b2zanusi, file="Data/FIA_GAM_ZANBI_DE_FSD.rds")
...
saveRDS(AbnMx, file="Data/FIA_bnMx_EV_FSD.rds")
saveRDS(BbnMx, file="Data/FIA_bnMx_DE_FSD.rds")
